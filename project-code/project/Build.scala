...
object GTEnginePlayBuild extends Build {
  ...
  val typesafeRepoUrl = "http://repo.typesafe.com/typesafe/ivy-releases/"
  ...
  object Resolvers {
      ...
      val typesafe = Resolver.url("Typesafe Repository", url(typesafeRepoUrl))(Resolver.ivyStylePatterns)
  }

  object Dependencies {

      val runtime = Seq(
        "kjetland"     %%   "gt-engine"            % "0.1.7.9",
        "play"         %%   "play"                 % "2.0-RC2" % "provided" notTransitive(),
        "play"         %%   "templates"            % "2.0-RC2" % "provided" notTransitive(),
